// CONTACT — Stone tablet you carve your message into
(function () {
  const { useState, useEffect, useRef } = React;

  function CarveField({ label, value, onChange, multi = false, placeholder = "" }) {
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        <Tag
          ...
          onChange={(e) => {
            ...
            if (e.target.value.length > value.length) window.SFX && window.SFX.carve();
          }}
  ...
  window.ContactPage = function ContactPage({ onBack }) {
    const data = window.PORTFOLIO_DATA.contact;
            ...
            <div style={{ position: "relative", padding: "76px 84px 70px", minHeight: 600 }}>
              {!sent ? (
                ...
                <div style={{
                  ...
                  <button onClick={() => { setSent(false); setForm({name:"",origin:"",message:""}); window.SFX && window.SFX.back(); }}
                    onMouseEnter={() => window.SFX && window.SFX.hover()}
                    style={{
                      cursor: "none",
                      marginTop: 12,
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 28, alignSelf: "center" }}>
            ...
            <div style={{
              ...
              <p style={{
                ...
                "However grim the Lands Between may seem, do not forget — there are those who pursue a fair land. Among them I count my correspondents."
              </p>
            </div>
          </div>
        </div>
